...
import org.javact.lang.JSMSendException;
...
public class SendCt extends FutureInitializer implements SendCtI {
	...
	/**
	 * Sending of standard messages. Try/Catch, imposed by RMI, is hidden from
	 * the programmer
	 *
	 * @param m the message
	...
	public void send(Message m, Agent target) {
		...
		try {
			((NetActor) target).receive(m);
		} catch (Exception e) {
			throw new SendException("Unable to send " + m + " to " + target, e);
		}
	}

	/**
	 * Sending of messages for which a reply can be requested. Try/Catch,
	 * imposed by RMI, is hidden from the programmer. The catched exception
	 * corresponds to a message sent several times.
	 *
	 * @param m the message
	 * @param target the target actor
	 *
	 * @throws JSMSendException if an error occured with the JSM protocol
	 * @throws SendException if m or target is null, or if an error occured while sending the message
	 */
	public void send(MessageWithReply m, Agent target) {
		...
		try {
			UnicastRemoteObject.exportObject(csab);
		} catch (RemoteException e) {
			throw new JSMSendException("Unable to export the csab", e);
		}
